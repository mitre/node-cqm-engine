...
{ Prefilter, AggregatePrefilter} = require("../lib/prefilter.js")
module.exports = class CQMExecutionEngine {
  ...
  buildPrefilter(measure, effective_date) {
    let prefilter = new AggregatePrefilter("and");
    let dc = {}
  ...
  filterCharacteristic(data_criteria,effective_date) {
    if (data_criteria['definition'] == 'patient_characteristic_birthdate') {
      if (data_criteria_in_population_ipp(measure, criteria_name)) {
        let filter = new AggregatePrefilter("and");
        if (data_criteria['temporal_references']) {
          let prefilter = new Prefilter({record_field: 'birthdate',
      ...
      return new Prefilter({record_field: 'gender', comparison: "$eq", desired_value: data_criteria["value"]["code"]})
    }
  }
