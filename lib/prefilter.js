...
module.exports.Prefilter = class Prefilter {
  ...
  /* Build a mongodb query filter based on the effective_time
  @param {number} effective_time -- date used to reference age based filters from
  @return {object} mongodb query filter hash
  */
  buildQueryHash(effective_time) {
    ...
    if (this.effective_time_based) {
      ...
      filter_value = new Date((et.getTime()/1000) * 1000);
    } else {
...
/*
Wrapper filter for conjunction based filters AND and OR.  This filter can contain simple prefiltes as well as other nested
aggregate filters
*/
module.exports.AggregatePrefilter = class AggregatePrefilter {
  /* @param {string} aggregate -- either 'and' or 'or'
  */
  ...
  /* Add a filter to this aggregate
   @param {PreFilter/AggregatePrefilter} filter -- the filter to add
   */
  addFilter(filter) {
    if (filter) {
